...
def fetch_csv(url):
    """Fetch CSV from URL with a timestamp to avoid caching."""
...
async def send_greeting(context: ContextTypes.DEFAULT_TYPE, user_id: int, name: str):
    ...
    try:
        ...
        logger.exception("Failed to send greeting to %s", user_id)
    ...
    if data.startswith("skip:"):
        ...
        if job:
            try:
                ...
                logger.debug("Failed to remove job for user %s", user_id)
            s["timer_job"] = None

        # stop poll if possible
        try:
...
async def start_quiz(context: ContextTypes.DEFAULT_TYPE, user_id: int, name: str):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=user_id, text="❌ Unable to fetch today's quiz. Please try again later.")
        return
...
async def send_question(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    try:
        ...
        logger.exception("Failed to send question text to %s", user_id)

    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Skip", callback_data=f"skip:{s['index']}")]])
    ...
    try:
        ...
        logger.exception("Failed to create poll for user %s", user_id)
        s["skipped"] = s.get("skipped", 0) + 1
    ...
    try:
        ...
        logger.exception("Failed to schedule timeout job for user %s", user_id)
        s["timer_job"] = None
...
async def finish_quiz(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if user_id not in daily_scores:
        daily_scores[user_id] = {"name": s["name"], "score": round(s["marks"], 2), "time": time_taken}
...
def main():
    ...
    # error handler to log exceptions in handlers
    async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
        logger.exception("Update caused exception: %s", update)
    ...
    try:
        ...
        logger.exception("Bot crashed on run_polling")
